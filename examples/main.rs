...
fn main() {
    ...
    icon.unwrap().save("output/notepad.png").unwrap();

    let icon = get_icon_by_path("C:\\Windows\\System32\\calc.exe");
    icon.unwrap().save("output/calc.png").unwrap();

    let icon = get_icon_by_path("C:\\Windows\\System32\\cmd.exe");
    icon.unwrap().save("output/cmd.png").unwrap();
    ...
    icon.unwrap().save("output/process.png").unwrap();
}
